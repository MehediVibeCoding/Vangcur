      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
        <div className="max-h-[85vh] w-full max-w-[420px] overflow-y-auto rounded-brand bg-white shadow-sh3">
          <div className="flex items-center justify-between border-b border-border-base px-5 py-4">
            <h3 className="font-display text-base font-bold text-ink">{t('এরপর কী হবে?')}</h3>
            ...
            {STEPS.map((s, idx) => (
              <div key={s.title} className="flex gap-3">
                ...
                <div className="pb-5 pt-1">
                  <div className="font-body text-[13px] font-bold text-ink">{t(s.title)}</div>
                  <div className="mt-0.5 font-body text-[12px] leading-[1.6] text-muted">{t(s.desc)}</div>
                </div>
              </div>
            ))}
            <div className="mt-1 rounded-[10px] bg-surface-muted px-3.5 py-3 font-body text-[12px] leading-[1.6] text-ink">
              {t('কোনো প্রশ্ন থাকলে ফুটারে দেওয়া নাম্বারে কল বা WhatsApp করুন — আমরা সবসময় পাশে আছি।')}
            </div>
          </div>
        </div>
      </div>
